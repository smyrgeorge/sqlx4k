...
class CommonSQLiteCipherByteArrayTests(
    ...
    fun `blob named param`() = runBlocking {
    ...
    // ---- Null blob binding (positional) ----
    fun `blob null positional binding`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob)").getOrThrow()
    ...
    // ---- Null blob binding (named) ----
    fun `blob null named binding`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob)").getOrThrow()
    ...
    // ---- Empty blob binding ----
    fun `blob empty array binding`() = runBlocking {
    ...
    fun `blob full byte range binding`() = runBlocking {
    ...
    // ---- Embedded null bytes ----
    fun `blob with embedded nulls`() = runBlocking {
    ...
    fun `blob insert and select roundtrip`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob not null)").getOrThrow()
    ...
    fun `blob large payload binding`() = runBlocking {
        val size = 8 * 1024
    ...
    // ---- UTF-8 text roundtrip via blob param ----
    fun `blob utf8 roundtrip via param`() = runBlocking {
    ...
    // ---- Reused named blob parameter ----
    fun `reused named blob param`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, a blob not null, b blob not null)").getOrThrow()
    ...
    // ---- Filter by blob equality ----
    fun `blob equality filter with prepared statement`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob not null)").getOrThrow()
    ...
    fun `batch insert blob and filter by id`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob not null)").getOrThrow()
    ...
    fun `blob single byte boundary values`() = runBlocking {
        val cases = listOf(
            byteArrayOf(0x00),
            byteArrayOf(0x7F),
    ...
    // ---- Update blob via named param ----
    fun `update blob via named param`() = runBlocking {
        val table = newTable()
        try {
            db.execute("create table $table(id integer primary key, data blob not null)").getOrThrow()
    ...
    // ---- Blob length via length() on bound param ----
    fun `blob param with length`() = runBlocking {
    ...
    // ---- Mixed: blob + other param types ----
    fun `blob mixed with other params`() = runBlocking {
        val table = newTable()
        try {
            db.execute(
                "create table $table(id integer primary key, label text not null, data blob not null, amount integer not null)"
            ).getOrThrow()
    ...
    fun `blob repeated byte pattern binding`() = runBlocking {

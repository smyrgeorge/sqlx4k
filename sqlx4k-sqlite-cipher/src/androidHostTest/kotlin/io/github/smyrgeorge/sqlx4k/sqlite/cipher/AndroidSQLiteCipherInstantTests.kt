...
@Config(sdk = [34])
class AndroidSQLiteCipherInstantTests {
    ...
    init {
        val dbFile = File(context.cacheDir, "sqlx4k-instant-tests.db").apply {
            if (exists()) delete()
        }
        db = sqliteCipher(
            context = context,
    ...
    private val runner = CommonSQLiteCipherInstantTests(db)

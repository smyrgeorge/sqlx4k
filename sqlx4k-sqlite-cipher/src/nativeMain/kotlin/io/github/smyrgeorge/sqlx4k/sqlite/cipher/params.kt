...
import sqlx4k.sqlite.cipher.PARAM_BLOB
import sqlx4k.sqlite.cipher.PARAM_INT
import sqlx4k.sqlite.cipher.PARAM_NULL
import sqlx4k.sqlite.cipher.PARAM_REAL
import sqlx4k.sqlite.cipher.PARAM_TEXT
import sqlx4k.sqlite.cipher.Sqlx4kSqliteCipherParam

/**
 * Allocates an FFI-ready array of [Sqlx4kSqliteCipherParam] inside the receiver [MemScope]
...
internal fun MemScope.allocParams(
    values: List<Any?>
): Pair<CPointer<Sqlx4kSqliteCipherParam>?, Int> {
    if (values.isEmpty()) return Pair(null, 0)
    val arr = allocArray<Sqlx4kSqliteCipherParam>(values.size)
    values.forEachIndexed { index, value -> bindParam(arr[index], value) }
    return Pair(arr, values.size)
}

private fun MemScope.bindParam(p: Sqlx4kSqliteCipherParam, value: Any?) {
    when (value) {

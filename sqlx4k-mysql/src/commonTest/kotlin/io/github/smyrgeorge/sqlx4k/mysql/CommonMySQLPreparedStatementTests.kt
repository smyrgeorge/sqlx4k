...
class CommonMySQLPreparedStatementTests(
    ...
    fun `null parameter binding`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, v_text text, v_int int)"
            ).getOrThrow()
    ...
    fun `datetime and uuid types as parameters`() = runBlocking {
        ...
        try {
            ...
            val datetime = LocalDateTime(2025, 3, 25, 7, 31, 43, 330068000)
            val instant = Instant.parse("2025-03-25T07:31:43.330068Z")
            val uuid = Uuid.parse("22d64ef8-f6b3-43da-8869-2ee9d31be9d5")

            val insert = Statement.create(
                "insert into $table(v_date, v_time, v_datetime, v_datetime_tz, v_uuid) values (:date, :time, :dt, :dttz, :uuid)"
            )
                .bind("date", date)
                .bind("time", time)
                .bind("dt", datetime)
            ...
            val row = db.fetchAll(
                Statement.create("select v_date, v_time, v_datetime, v_datetime_tz, v_uuid from $table where v_uuid = :uuid")
                    .bind("uuid", uuid)
            ).getOrThrow().first()
            ...
            assertAll {
                ...
                assertThat(row.get(2).asLocalDateTime()).isEqualTo(datetime)
    ...
    fun `type cast preservation with params`() = runBlocking {
        // In MySQL there is no :: cast syntax; verify params work correctly inside CAST() expressions
        val select = Statement.create("select CAST(? AS CHAR) as casted, CAST(:val AS SIGNED) as named_cast")
            .bind(0, "hello")
            .bind("val", 42)
        val row = db.fetchAll(select).getOrThrow().first()
    ...
    fun `reused named parameter`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, a int, b int)"
            ).getOrThrow()
    ...
    fun `string values with special characters`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, v_text text)"
            ).getOrThrow()
    ...
    fun `set expansion with IN clause`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, v int not null)"
            ).getOrThrow()
    ...
    fun `longArray expansion with IN clause`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, v bigint not null)"
            ).getOrThrow()
    ...
    fun `collection expansion mixed with scalar params`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, name text not null, score int not null)"
            ).getOrThrow()
    ...
    fun `custom type mixed with primitives and collections`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, label text not null, amount int not null, score int not null)"
            ).getOrThrow()
    ...
    fun `reused named param with custom type`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, col_a text not null, col_b text not null)"
            ).getOrThrow()
    ...
    fun `custom type with type cast`() = runBlocking {
        // Custom type + MySQL CAST() must not confuse the scanner
        val select = Statement.create("select CAST(:tag AS CHAR) as casted, CAST(? AS SIGNED) as amount")
            .bind("tag", Tag("hello"))
            .bind(0, Money(7, "GBP"))
        val row = db.fetchAll(select).getOrThrow().first()
    ...
    fun `enum as parameter`() = runBlocking {
        ...
        try {
            db.execute(
                "create table $table(id int auto_increment primary key, status text not null)"
            ).getOrThrow()
    ...
    fun `char as parameter`() = runBlocking {
        val select = Statement.create("select CAST(? AS CHAR) as ch")
            .bind(0, 'Z')
        val row = db.fetchAll(select).getOrThrow().first()
    ...
    fun `byte as parameter`() = runBlocking {
        val select = Statement.create("select CAST(? AS SIGNED) as b")
            .bind(0, 5.toByte())
        val row = db.fetchAll(select).getOrThrow().first()

...
use sqlx::{Acquire, Column, Executor, MySql, Row, Transaction, TypeInfo, ValueRef};
use sqlx4k::{
...
fn sqlx4k_mysql_result_of(result: Result<Vec<MySqlRow>, sqlx::Error>) -> Sqlx4kResult {
    match result {
        Ok(rows) => {
            ...
            let rows: Vec<Sqlx4kRow> = rows.iter().map(|r| sqlx4k_mysql_row_of(r)).collect();
            let size = rows.len();
...
fn sqlx4k_mysql_schema_of(row: &MySqlRow) -> Sqlx4kSchema {
...
fn sqlx4k_mysql_row_of(row: &MySqlRow) -> Sqlx4kRow {
    let columns = row.columns();
    if columns.is_empty() {
        Sqlx4kRow::default()
    } else {

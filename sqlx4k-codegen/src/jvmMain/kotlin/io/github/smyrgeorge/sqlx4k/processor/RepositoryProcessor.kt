...
class RepositoryProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        validatedRepos.forEach { repo ->
            ...
            // Collect all parameter types that need to be imported
            val parameterTypeImports = collectParameterTypeImports(fnsAll, outputPackage)
    ...
    private fun emitCrudMethods(
        ...
        if (dialect != Dialect.MySQL) {
            ...
            file += "\n"
            file += "    /**\n"
            file += "     * Inserts multiple $domainSimpleName entities into the database in a single batch operation.\n"
            file += "     *\n"
            ...
            file += "     * @param entities The collection of $domainSimpleName entities to insert\n"
            file += "     * @return Result containing the list of inserted ${domainSimpleName}s with generated values,\n"
            file += "     *         or an error if the batch insert operation fails\n"
            file += "     */\n"
            if (useContextParameters) {
                file += "    context(context: QueryExecutor)\n"
                file += "    override suspend fun batchInsert(entities: Iterable<$domainQn>) = run {\n"
            } else {
                file += "    override suspend fun batchInsert(context: QueryExecutor, entities: Iterable<$domainQn>) = run {\n"
            }

            file += "        val items = entities.toList()\n"
            file += "        if (items.isEmpty()) return@run Result.success(emptyList())\n"
            file += "\n"

            // Apply preInsertHook to each entity if overridden
            val hasPreInsertHook = isHookOverridden(repo, "preInsertHook")
            if (hasPreInsertHook) {
                file += "        val processedItems = items.map { entity -> preInsertHook(context, entity) }\n"
                file += "        val statement = processedItems.insert()\n"
            } else {
                file += "        val statement = items.insert()\n"
            }

            val entitiesVar = if (hasPreInsertHook) "processedItems" else "items"
            if (hasAroundQueryHook) {
                file += "        aroundQuery(\"batchInsert\", statement) {\n"
                file += "            context.fetchAll(statement)\n"
                file += "        }.map { rs ->\n"
            } else {
                file += "        context.fetchAll(statement).map { rs ->\n"
            }
            file += "            $entitiesVar.applyInsertResult(rs.rows, context.encoders)\n"
            file += "        }"

            val hasAfterInsertHook = isHookOverridden(repo, "afterInsertHook")
            if (hasAfterInsertHook) {
                file += ".map { resultList ->\n"
                file += "            resultList.map { entity -> afterInsertHook(context, entity) }\n"
                file += "        }\n"
            } else {
                file += "\n"
            }
            ...
            // MySQL doesn't support batch insert - generate error method
            logger.info("[RepositoryProcessor] Emitting unsupported CRUD method: batchInsert(Iterable<$domainQn>) for MySQL")
            file += "\n"
            file += "    /**\n"
            file += "     * Batch insert is not supported for MySQL dialect.\n"
            file += "     *\n"
            file += "     * MySQL does not support multi-row INSERT statements with RETURNING clause,\n"
            file += "     * which is required for batch insert operations.\n"
            file += "     *\n"
            if (!useContextParameters) {
                file += "     * @param context The query executor (database connection or transaction)\n"
            }
            ...
            if (useContextParameters) {
                file += "    context(context: QueryExecutor)\n"
                file += "    override suspend fun batchInsert(entities: Iterable<$domainQn>)"
            } else {
                file += "    override suspend fun batchInsert(context: QueryExecutor, entities: Iterable<$domainQn>)"
            }
            file += if (useArrow) {
                ": DbResult<List<$domainQn>> = throw UnsupportedOperationException(\"Batch insert is not supported for MySQL dialect (no multi-row INSERT with RETURNING support)\")\n"
            } else {
                ": Result<List<$domainQn>> = throw UnsupportedOperationException(\"Batch insert is not supported for MySQL dialect (no multi-row INSERT with RETURNING support)\")\n"
            }
        }
        ...
        if (dialect != Dialect.MySQL) {
            ...
            file += "\n"
            file += "    /**\n"
            file += "     * Updates multiple $domainSimpleName entities in the database in a single batch operation.\n"
            file += "     *\n"
            ...
            file += "     * @param entities The collection of $domainSimpleName entities to update\n"
            file += "     * @return Result containing the list of updated ${domainSimpleName}s,\n"
            file += "     *         or an error if the batch update operation fails\n"
            file += "     */\n"
            if (useContextParameters) {
                file += "    context(context: QueryExecutor)\n"
                file += "    override suspend fun batchUpdate(entities: Iterable<$domainQn>) = run {\n"
            } else {
                file += "    override suspend fun batchUpdate(context: QueryExecutor, entities: Iterable<$domainQn>) = run {\n"
            }
            ...
            // Apply preUpdateHook to each entity if overridden
            val hasPreUpdateHook = isHookOverridden(repo, "preUpdateHook")
            if (hasPreUpdateHook) {
                file += "        val processedItems = items.map { entity -> preUpdateHook(context, entity) }\n"
                file += "        val statement = processedItems.update()\n"
            } else {
                file += "        val statement = items.update()\n"
            }

            val updateEntitiesVar = if (hasPreUpdateHook) "processedItems" else "items"
            if (hasAroundQueryHook) {
            ...
            if (hasAfterUpdateHook) {
                file += ".map { resultList ->\n"
                file += "            resultList.map { entity -> afterUpdateHook(context, entity) }\n"
                file += "        }\n"
            } else {
                file += "\n"
            }
            ...
            if (useContextParameters) {
                file += "    context(context: QueryExecutor)\n"
                file += "    override suspend fun batchUpdate(entities: Iterable<$domainQn>)"
            } else {
    ...
    /**
     * Collects all qualified type names from method parameters that need to be imported.
     *
     * This function recursively extracts type information from method parameters, including
     * nested generic types, and filters out types that don't need explicit imports
     * (e.g., kotlin.*, java.lang.*, same package types, and types already imported by default).
     *
     * @param functions The list of function declarations to analyze.
     * @param outputPackage The package of the generated file (types in this package don't need imports).
     * @return A set of fully qualified type names that should be imported.
     */
    private fun collectParameterTypeImports(
        ...
        // Types already imported by default in the generated file
        ...
        fun collectFromType(type: KSType) {
            ...
            // Note: kotlin.uuid.* types DO need imports, so we can't blanket-skip all kotlin.*
            ...
            if (isBuiltinKotlinType ||
                declaration.packageName.asString() == outputPackage ||
                qualifiedName in alreadyImported
            ) {
                // Still need to process type arguments for generic types like List<Uuid>
                type.arguments.forEach { arg ->
                    arg.type?.resolve()?.let { collectFromType(it) }
                }
                return
            }
        ...
        functions.forEach { fn ->
            fn.parameters.forEach { param ->
                collectFromType(param.type.resolve())
            }
        }

...
//!
//! The bundled `sqlite3.c` is compiled with large-file support, so on Linux it references the glibc
//! `fcntl64` symbol. Kotlin/Native links against an older bundled glibc sysroot that only exports
//! `fcntl`, leaving `fcntl64` undefined. We provide `fcntl64` here, forwarding to `fcntl` (which
//! Kotlin/Native does resolve).
//!

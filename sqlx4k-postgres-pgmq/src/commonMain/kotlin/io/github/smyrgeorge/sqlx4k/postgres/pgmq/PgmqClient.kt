...
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.bindTopic as bindTopicWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.delete as deleteWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.pop as popWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.read as readWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.send as sendWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.sendTopic as sendTopicWithContext
import io.github.smyrgeorge.sqlx4k.postgres.pgmq.impl.extensions.setVt as setVtWithContext
...
class PgmqClient(
     ...
     */
    suspend fun installExtensions(path: String) {
    ...
    suspend fun installExtension(file: Pair<String, String>) {
        ...
        if (statements.isEmpty()) SQLError(SQLError.Code.Migrate, "Extension file ${file.first} is empty.").raise()
    ...
    suspend fun create(queue: Queue): Result<Unit> = runCatching {
        ...
        suspend fun enableNotifyInsert(queue: String, throttleNotifyInterval: Duration): Result<Unit> {
            ...
            val statement =
                Statement.create(sql).bind(0, queue).bind(1, throttleNotifyInterval.inWholeMilliseconds.toInt())
            return tx.fetchAll(statement, UnitRowMapper).toSingleUnit()
        }
    ...
    ): Result<Long> = with(pg) { sendWithContext(queue, message, headers, delay) }
    ...
    suspend fun send(
        ...
        headers: Map<String, String> = emptyMap(),
        delay: Duration = 0.seconds
    ): Result<List<Long>> = with(pg) { sendWithContext(queue, messages, headers, delay) }
    ...
    suspend fun pop(
        queue: String,
        quantity: Int = 1
    ): Result<List<Message>> = with(pg) { popWithContext(queue, quantity) }
    ...
    suspend fun read(
        queue: String,
        quantity: Int = 1,
        vt: Duration = 30.seconds
    ): Result<List<Message>> = with(pg) { readWithContext(queue, quantity, vt) }
    ...
    suspend fun archive(
        queue: String,
        id: Long
    ): Result<Boolean> = with(pg) { archiveWithContext(queue, id) }
    ...
    suspend fun archive(
        queue: String,
        ids: List<Long>
    ): Result<List<Long>> = with(pg) { archiveWithContext(queue, ids) }
    ...
    suspend fun delete(
        queue: String,
        id: Long
    ): Result<Boolean> = with(pg) { deleteWithContext(queue, id) }
    ...
    suspend fun delete(
        queue: String,
        ids: List<Long>
    ): Result<List<Long>> = with(pg) { deleteWithContext(queue, ids) }
     ...
     * @param vt The new visibility timeout value as the Duration.
    ...
    suspend fun setVt(
        ...
        id: Long,
        vt: Duration
    ): Result<Long> = with(pg) { setVtWithContext(queue, id, vt) }
     ...
     */
    suspend fun bindTopic(
        pattern: String,
        queueName: String
    ): Result<Unit> = with(pg) { bindTopicWithContext(pattern, queueName) }
    ...
    suspend fun sendTopic(
        ...
        headers: Map<String, String> = emptyMap(),
        delay: Duration = 0.seconds
    ): Result<Long> = with(pg) { sendTopicWithContext(routingKey, message, headers, delay) }
